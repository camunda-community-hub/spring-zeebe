...
@ConfigurationProperties(prefix = "camunda.tasklist.client")
public class TaskListClientConfigurationProperties extends Client {
  ...
  public String getTaskListUrl() {
    if (url != null) {
      LOG.debug("Connecting to Camunda TaskList on URL: " +url);
      return url;
    } else if (clusterId != null) {
      String url = "https://" + region + "." + getFinalBaseUrl() + "/" + clusterId + "/";
      LOG.debug("Connecting to Camunda TaskList SaaS via URL: " + url);
      return url;
    }
    throw new IllegalArgumentException(
      "In order to connect to Camunda TaskList you need to specify either a SaaS clusterId or a TaskList URL.");
  }
